...
class SonarQubeClient:
    ...
    def get_file_issues_information(
        self,
        project_key: str,
        file_path: str,
        branch: Optional[str] = None,
        ...
        while True:
            ...
            for issue in issues:
                ...
                if isinstance(rule_response, dict) and "error" in rule_response:
                    logger.error(f"Failed to fetch rule {issue.get('rule')}: {rule_response.get('details', 'Unknown error')}")
                    result["errors"].append(f"Rule {issue.get('rule')} fetch failed: {rule_response.get('details', 'Unknown error')}")
                else:
                ...
                if isinstance(snippet_response, dict) and "error" in snippet_response:
                    ...
                    if snippet:
                        start_line = min(line.get("line", float('inf')) for line in snippet)
                        end_line = max(line.get("line", 0) for line in snippet)
                        issue_data["source_snippet"] = {
                            "startLine": start_line if start_line != float('inf') else None,
                            "endLine": end_line if end_line != 0 else None,
                            "code": [
                                {"line": line.get("line"), "code": line.get("code")} for line in snippet
                            ]
                        }
                    else:
                        issue_data["source_snippet"] = {
